  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[.9fr_1.1fr]">
      <aside className="hidden bg-navy p-12 text-white lg:flex lg:flex-col">
        <Link href="/" className="flex items-center gap-2 font-semibold"><ShieldAlert className="text-safety" />RakshAI</Link>
        <div className="my-auto max-w-sm"><p className="text-sm font-medium text-sky-200">Careful guidance, ready when you need it.</p><h1 className="mt-4 text-4xl font-semibold leading-tight">Emergency support that keeps you oriented.</h1><p className="mt-5 leading-7 text-slate-300">RakshAI is designed to help you gather useful information and take safer next steps—not to replace professional emergency care.</p></div>
        <p className="text-sm text-slate-400">If someone is in immediate danger, call your local emergency number first.</p>
      </aside>
      <section className="flex items-center justify-center bg-white px-6 py-12"><div className="w-full max-w-md"><Link href="/" className="mb-10 flex items-center gap-2 font-semibold text-navy lg:hidden"><ShieldAlert className="text-safety" />RakshAI</Link><p className="text-sm font-medium text-sky-700">{isRegistration ? "Create your account" : "Welcome back"}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight text-navy">{isRegistration ? "Be prepared, without the panic." : "Sign in to RakshAI"}</h2><p className="mt-3 text-slate-600">{isRegistration ? "Keep your emergency history and contacts in one private place." : "Access your emergency sessions and contacts."}</p>
        ...
        <p className="mt-6 text-center text-sm text-slate-600">{isRegistration ? "Already have an account?" : "New to RakshAI?"} <Link className="font-semibold text-sky-700 hover:underline" href={isRegistration ? "/login" : "/register"}>{isRegistration ? "Sign in" : "Create an account"}</Link></p>
      </div></section>
    </main>
  );
}
